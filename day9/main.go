...
func (s step) String() string {
	return fmt.Sprintf("%s %d", string(s.direction), s.amount)
}

func first(steps []step) int {
	head := maps.Coordinate{X: 0, Y: 0}
	tail := maps.Coordinate{X: 0, Y: 0}
...
func second(steps []step) int {
	var head maps.Coordinate
	tails := make([]maps.Coordinate, 9)
	visits := make(map[maps.Coordinate]bool)
	tailIdx := len(tails) - 1
	visits[tails[tailIdx]] = true
...
func move(head, tail maps.Coordinate) maps.Coordinate {
	...
	switch {
	case distanceX == 0 && distanceY == 2:
		tail.Y += yMove

	case distanceX == 2 && distanceY == 0:
		tail.X += xMove
